...
$(document).on('keyup','.val',function(e){
	...
	if((typeof(val_type)!='undefined') && (val_type.trim()!=''))
	{
		...
		if(val_type.indexOf("max_value") >= 0)
		{
			max_val=val_type.split('max_value');
		...
		if((val_type.indexOf('uppercase') >= 0) )
		{
...
function isTextSelected(input) {
    if (typeof input[0].selectionStart == "number") {
        ...
        input[0].focus();
        return document.selection.createRange().text == input[0].value;
    }
}
			...
			if(val_type.indexOf("max_digit") >= 0)
			{
				...
				if((typeof(max_digit[1])!='undefined') && (max_digit[1].trim()!=''))
				{
					...
					if(!isNaN((max_digit)))
					{
						...
						if(rule_set==1)
						{
							if((($(this).val().length)<(max_digit)|| ((eve==8))||((eve==9))||((eve==46)) || textSelected )&&(val_check==1))
								val_check=1;
							else 
								val_check=0;
								
						}
						else if((($(this).val().length)<(max_digit)|| ((eve==8))||((eve==9))||((eve==46)) || textSelected ))
							val_check=1;
						else 
							val_check=0;
			...
			if((val_type.indexOf("decimal") >= 0))
			{
				...
				if((typeof(dec_digit[1])!='undefined') && (dec_digit[1].trim()!=''))
				{
					...
					if(!isNaN((dec_digit)))
					{
						var input = $(this).val();
						//console.log(eve);
						if ((eve != 46 || input.indexOf('.') != -1) && (eve < 48 || eve > 57)) {
							return false;
...
function validat(typ,data)
{
	...
	filterlocation = /^-?([1-8]?[1-9]|[1-9]0)\\.{1}\\d{1,6}/,
	val_check=0;
	//if(typ==0) data = $(this);
	//else 
	data = $(data);
			...
			else
			{
				...
				else
				{
					...
					if((val_type.indexOf('website') >= 0))
					{
						...
						{
							val_check=1;
							$(this).after('<span class="error" style="display:block;color: #ff0202;position: absolute;">* Please Enter Valid Url </span>');
							$(this).addClass('has-error');
						}
					}
					if((val_type.indexOf('location') >= 0))
					{
						if (!(filterlocation.test($(this).val())) || ($(this).val().trim()==''))
						{
							val_check=1;
							$(this).addClass('has-error');
						}
					...
					if((val_type.indexOf('min_char') >= 0))
					{
						...
						if((typeof(min_digit[1])!='undefined') && (min_digit[1].trim()!=''))
						{
							...
							{	
								...
								{
									val_check=1;
									if(val_type.indexOf("num") >= 0 || val_type.indexOf("mobile") >= 0 || val_type.indexOf("decimal") || val_type.indexOf("float_num"))
